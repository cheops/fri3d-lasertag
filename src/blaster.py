...
class Enum:
    ...
    def lookup(cls, number):
        if not cls._lookup_table:
            cls._lookup_table = {getattr(cls, x): x for x in dir(cls) if
                                 not x.startswith("_") and not callable(getattr(cls, x))}
...
class DataPacket:
    ...
    @property
    def crc(self):
        return (self._raw & 0b1111000000000000) >> 12
    ...
    def __repr__(self):
        return (
            f"{self.team_str}, {self.trigger=}, {self.command_str}, {self.parameter=}, {self.crc=}, {self.calculate_crc()=}")
...
class Blaster:
    ...
    def forward_ir_shot(self):
        ...
        if p.command not in [Command.heal, Command.shoot]: return
        self._blaster_link.transmit_packet(p)
